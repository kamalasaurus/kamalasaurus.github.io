...
The caveat being a `Promise` is only yieldable <b>IF</b> you `return` out of all of its accessible branches.  Meaning every block that gets passed through has to `return` control to its callee.  Turtles all the way down.
